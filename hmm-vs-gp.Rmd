...
By fitting both the Gaussian process model and the hidden Markov model to neural data, we can use bride sampling to find which model is more likely to explain the neural activity.  However, these models aren't complete yet!  In the next notebook we'll build versions of the model which are able to handle data from both mutliple trials and multiple subjects.
