...
# Store data
hmm_data = list(N=N, M=Nt, x=x, y=y)
```
...
check_treedepth(fit_gp_to_gp)
check_energy(fit_gp_to_gp)
check_divergences(fit_gp_to_gp)
...
check_treedepth(fit_hmm_to_hmm)
check_energy(fit_hmm_to_hmm)
check_divergences(fit_hmm_to_hmm)
```
...
Here is the Stan routine for the multilevel Gaussian process model.  The main difference is that now we have to compute a covariance matrix for each subject individually, and of course now we have population distributions and individual parameters for each subject.

```{r}
writeLines(readLines("lgp_multilevel.stan"))
```

TODO: actually might have to parameterize the gamma dists as mean and var, instead of alpha and beta (better sampling for stan w/ a multilevel model!)
...
TODO: generate data from multiple subjects w/ multiple trials
