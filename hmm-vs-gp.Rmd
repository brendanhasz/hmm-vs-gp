...
## Stan models for multiple trials
...
Let's generate some test data which contiains multiple time series (to emulate data on multiple trials).
...
And now we can take a look at all the latent functions we generated (lines)
...
title('A gaggle of Gaussian processes')
```


## Multilevel Stan models for both multiple subjects and multiple trials

Another problem with the Stan model so far is they only handle a single subject.  We want our models to include mixed effects (inter-subject variablility), and so we'll build multilevel versions of both the Gaussian process and hidden Markov models.
