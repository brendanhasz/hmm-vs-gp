...
plot(rho_prior, 0, 3, xlab='Rho', ylab='',
...
plot(alpha_prior, 0, 6, xlab='Alpha', ylab='',
...
plot(sigma_prior, 0, 6, xlab='Sigma', ylab='',
...
TODO: or not. hm. something's wrong here... log marginal likelihood for gp->gp_data is tiny and for hmm->gp_data is huge... The mean log posterior value for gp->gp_data is -59.25 while the mean log post val for hmm->gp_data is +31.38.  sigh.  I'll bet it's because the p(x|params) for any individual datapoint for the HMM model can be large (>1, a beta dist w/ alpha=1 and beta=4.6 is ~5 near zero). So if you multiply those all together for the p(x|params) for all the data you get a huge number, whereas for the GP it's a multivariate normal dist in inf-space so likely to be small anywhere... Maybe.  Ideas:

* HMM model might just be specified incorrectly.  Should double-check and test.
* Or, another possibility is that since your "parameters" of the GP are actually the hyperparameters, you should be using (what's normally referred to as) the *marginal likelihood* of the GP (given those hyperparams) as the likelihood.  See line 7 of Algorithm 2.1 (pg 19) in Rasmussen & Williams' GP book.  ATM this seems most likely.
* ~~Might have to add a normalizer to the HMM? Integral of all possible y combinations for the GP is 1 (because it's a multivariate normal), but integral of all possible y combos for the HMM is larger than that I think.~~  Nope actually, it's 1 - I checked.
* Maybe it's not valid to compare the marginal probabilities of models in completely different classes?  All the examples in bridgesampling docs are comparing models of the same class.

```{r}
# Fit GP to HMM-generated data
# TODO: reformat data?
#fit_gp_to_hmm = # TODO: stan fit of GP to HMM-generated data
  
# Fit HMM to GP-generated data
# TODO: reformat data?
#fit_hmm_to_gp = # TODO: stan fit of HMM to GP-generated data
  
# Look at Bayes factor for GP-generated data
#bridge_gp_gp = bridge_sampler(fit_gp)
#bridge_hmm_gp = bridge_sampler(fit_hmm_to_gp)
#print("Bayes factor of GP to HMM on GP-generated data")
#bf(bridge_gp_gp, bridge_hmm_gp)

# Look at Bayes factor for HMM-generated data
