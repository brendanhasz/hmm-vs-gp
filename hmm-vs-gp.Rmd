...
Our basic single-level Gaussian process has three parameters: the length scale ($\rho$), the signal standard deviation ($\alpha$, a.k.a. the marginal or output standard deviation), and the noise standard deviation ($\sigma$).  All three are constrained to be greater than zero.  We'll use a [Gamma distribution](https://en.wikipedia.org/wiki/Gamma_distribution) to model the population distribution for all three parameters. Furthermore, we'll put a prior on the means of these population-level distributions which is the same as the priors we were previously using on the parameters themselves (in the single-subject versions of the mdoels).

![Multilevel Gaussian Process](img/multilevel_gaussian_process.svg)

The length scale parameter ($\rho$) for each subject $s$ is drawn from a population gamma distribution with parameters $\alpha=\rho_\alpha$ and $\beta=\rho_\beta$.

$$
\forall s, ~ \rho_s \sim \text{Gamma}(\rho_\alpha, \rho_\beta)
$$

The mean of this population distribution is drawn from an inverse gamma distribution with $\alpha=2$ and $\beta=0.5$ (which was the prior on the $\rho$ parameter in the single-subject models).

$$
\text{E}[\text{Gamma}(\rho_\alpha, \rho_\beta)] = \frac{\rho_\alpha}{\rho_\beta} \sim \text{InvGamma}(2, 0.5)
$$

The signal standard deviation parameter ($\alpha$) for each subject $s$ is drawn from a population gamma distribution with parameters $\alpha=\alpha_\alpha$ and $\beta=\alpha_\beta$.

$$
\forall s, ~ \alpha_s \sim \text{Gamma}(\alpha_\alpha, \alpha_\beta)
$$

The mean of this population distribution is drawn from a half-normal distribution with $\mu=0$ and $\sigma=2$ (which was the prior on the $\alpha$ parameter in the single-subject models).

$$
\text{E}[\text{Gamma}(\alpha_\alpha, \alpha_\beta)] = \frac{\alpha_\alpha}{\alpha_\beta} \sim \text{HalfNormal}(0, 2)
$$

Finally, the noise standard deviation parameter ($\sigma$) for each subject $s$ is drawn from a population gamma distribution with parameters $\alpha=\sigma_\alpha$ and $\beta=\sigma_\beta$.

$$
\forall s, ~ \sigma_s \sim \text{Gamma}(\sigma_\alpha, \sigma_\beta)
$$

And the mean of this population distribution is drawn from a half-normal distribution with $\mu=0$ and $\sigma=1$ (which was the prior on the $\alpha$ parameter in the single-subject models).

$$
\text{E}[\text{Gamma}(\sigma_\alpha, \sigma_\beta)] = \frac{\sigma_\alpha}{\sigma_\beta} \sim \text{HalfNormal}(0, 1)
$$

Here is the Stan routine for the multilevel Gaussian process model.  The main difference is that now we have to compute a covariance matrix for each subject individually, and now we have population distributions from which individual subject's parameters are drawn.
...
The single-subject hidden Markov model has four parameters: the recurrent transition probabilities for state 1 ($\phi_{1,1}$) and state 2 ($\phi_{2,2}$), along with the observation parameters for state 1 ($\theta_1$) and state 2 ($\theta_2$).  The transition probabilities ($\phi_{i,i}$) are constrained between 0 and 1, so we'll use a [Beta distribution](https://en.wikipedia.org/wiki/Beta_distribution) to model their population distribution.  On the other hand, the observation parameters ($\theta_i$) are constrained to be greater than zero, so we'll use a gamma distribution to model their population distribution.  Like with the Gaussian process, we'll put a prior on the means of the population distributions which is identical to the prior we used on the raw parameters in the single-subject versions of the models.

![Multilevel Hidden Markov Model](img/multilevel_hidden_markov_model.svg)

The transition probability parameters ($\phi_{i,i}$) for each subject $s$ are drawn from population beta distributions with parameters $\alpha=\phi_{\alpha,i}$ and $\beta=\phi_{\beta,i}$.  

$$
\forall s, ~ \phi_{i,i,s} \sim \text{Beta}(\phi_{\alpha,i}, \phi_{\beta,i})
$$

Where $\phi_{i,i,s}$ is the $\phi_{i,i}$ parameter for subject $s$.

The means of these population distributions are drawn from a beta distribution with $\alpha=1.2$ and $\beta=1.2$ (which was the prior on the $\phi_{i,i}$ parameter in the single-subject models).

$$
\text{E}[\text{Beta}(\phi_{\alpha,i}, \phi_{\beta,i})] = \frac{\phi_{\alpha,i}}{\phi_{\alpha,i}+\phi_{\beta,i}} \sim \text{Beta}(1.2, 1.2)
$$

The observation parameters ($\theta_i$) for each subject $s$ are drawn from population gamma distributions with parameters $\alpha=\theta_{\alpha,i}$ and $\beta=\theta_{\beta,i}$.  

$$
\forall s, ~ \theta_{i,s} \sim \text{Gamma}(\theta_{\alpha,i}, \theta_{\beta,i})
$$

Where $\theta_{i,s}$ is the $\theta_i$ parameter for subject $s$.

The means of these population distributions are drawn from a gamma distribution with $\alpha=2$ and $\beta=2$ (which was the prior on the $\theta_i$ parameter in the single-subject models).

$$
\text{E}[\text{Gamma}(\theta_{\alpha,i}, \theta_{\beta,i})] = \frac{\theta_{\alpha,i}}{\theta_{\beta,i}} \sim \text{Gamma}(2, 2)
$$

Here is the Stan routine for the multilevel hidden Markov model.

```{r}
writeLines(readLines("hmm_multilevel.stan"))
```

I tried implementing several optimizations, including pre-computing logit(y), writing a custom user-defined Stan function to compute the covariance matrix which assumed linearly-spaced x values, etc.  However, none of the optimizations actually ended up making the sampling run any faster!  Stan's pretty fast as is.  Well, maybe not fast.  Optimized, at least.
...
TODO: maybe make the priors on the pop dist variances t-dists instead of half-normals?

TODO: update the math and model diagrams above!!! including the non-centered parameterizations, and why. Include link to: Check out this [case study](http://mc-stan.org/users/documentation/case-studies/divergences_and_bias.html)
...
#fit_hmm_to_hmm = stan(file='hmm_multilevel.stan', data=hmm_data, seed=seed+1000)
fit2 = stan(file='hmm_multilevel.stan', data=hmm_data, seed=seed+1000)
```
...
The diagnostics look mostly good, however there were some divergent transitions in some cases.  However, there are a very small proportion of divergent transitions (less than half a percent of samples).

TODO: check if divergences concentrate anywhere in parameter space - if not, we good
...
interval_density(posterior$rho_m, xlab="Population mean rho")
abline(v=rho_mu, col=c_dark, lwd=3)

interval_density(posterior$alpha_m, xlab="Population mean alpha")
abline(v=alpha_mu, col=c_dark, lwd=3)

interval_density(posterior$sigma_m, xlab="Population mean sigma")
abline(v=sigma_mu, col=c_dark, lwd=3)
```
...
interval_density(posterior$phi_m[,1], xlab="Population mean phi[1,1]")
abline(v=phi_mu, col=c_dark, lwd=3)

interval_density(posterior$phi_m[,2], xlab="Population mean phi[2,2]")
abline(v=phi_mu, col=c_dark, lwd=3)

interval_density(posterior$theta_m[,1], xlab="Population mean theta[1]")
abline(v=theta_mu, col=c_dark, lwd=3)

interval_density(posterior$theta_m[,2], xlab="Population mean theta[2]")
abline(v=theta_mu, col=c_dark, lwd=3)
```

TODO: ensure you recover correct INDIVIDUAL params for models fit to themselves

TODO: ensure bayes factors favor models fit to themselves

TODO: ensure marginal likelihoods are higher for each model when fit to themselves
